...
认证使用短期 JWT access token 和服务端 refresh session。access token 携带 `sub=userId`、`sid=sessionId` 和过期时间；请求进入后端时会校验 token、session 是否撤销或过期、用户是否启用，并按用户当前启用角色实时加载权限。

refresh token 只以哈希形式保存在 `auth_refresh_tokens`。刷新接口会轮换 refresh token；退出登录撤销当前 session；用户自助改密会撤销其它 session；管理员重置密码和停用用户会撤销该用户全部 session。开发令牌旁路默认关闭，仅测试或本地显式开启时可用。
