...
邮件找回密码通过 SMTP 发送重置链接。后端只保存一次性重置令牌哈希，默认 30 分钟有效；确认重置后会标记令牌已使用并撤销该用户全部 refresh session。本地 Docker Compose 使用 Mailpit 捕获邮件，生产环境应切换为企业邮箱或云邮件服务 SMTP，并配置发信域名的 SPF、DKIM 和 DMARC。
