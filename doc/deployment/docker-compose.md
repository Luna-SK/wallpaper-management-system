...
邮件找回密码由 `APP_MAIL_*` 环境变量控制。生产部署应设置 `APP_MAIL_ENABLED=true`，并配置 SMTP 主机、端口、账号、密码、发件人、TLS 开关和 `APP_MAIL_FRONTEND_BASE_URL`。`APP_MAIL_FRONTEND_BASE_URL` 用于生成重置链接，必须填写用户实际访问的前端地址；`APP_MAIL_PASSWORD_RESET_TOKEN_TTL` 默认 `30m`。
