...
def prepare_import_plan(
    ...
    *,
    ...
    for image in scanned_images:
        ...
        if retry_failed:
            if matching_previous and matching_previous.status == STATUS_FAILED:
                ...
                records_by_path[image.relative_path] = ImportRecord.from_image(image, STATUS_SKIPPED_COMPLETED)
            else:
                records_by_path[image.relative_path] = ImportRecord.from_image(image, STATUS_PLANNED)
            continue
