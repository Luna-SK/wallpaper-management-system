...
- 已启动的新项目后端服务
- 账号至少需要 `image:upload`、`image:view`、`image:edit` 权限，并能访问分类与标签列表接口
...
报告还会包含 `desiredTitle`、`titleUpdated`、`titleError` 列。脚本会按标签生成友好标题，例如 `破洞001`、`破洞002`，多个标签会生成 `水渍-油渍-污渍001`。编号按相同标签组合独立递增，超过 `999` 后会自然扩展为 `1000`、`1001`。

真实上传时，脚本会对新上传图片、后端判重命中的既有图片，以及 `--resume` 跳过但 checkpoint 中已有 `imageId` 的图片覆盖标题。标题覆盖前会先读取图片详情，并带着原状态、分类和标签一起更新，避免误清空已有分类或标签。
...
- 已确认入库的图片不会回滚；下次可使用 `--resume --no-dry-run` 继续补齐未完成项或标题。
...
- 本地重复图片默认只报告并跳过，不会为同一后端图片反复覆盖不同标题，也不会修改已有图片分类或标签。
